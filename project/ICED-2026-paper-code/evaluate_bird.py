...
def _get_conn(db_file):
    ...
    if conn is None:
        ...
        _conn_cache[db_file] = conn
    return conn

# 比较两个sql的执行结果
def compare_sql2(question_id, db_file, question, ground_truth, pred_sql):
    ...
    try:
        conn.execute("BEGIN TRANSACTION;")
        p = _strip_semicolon(pred_sql)
        g = _strip_semicolon(ground_truth)
        sql_eq = (
            "SELECT CASE "
            "WHEN EXISTS(SELECT 1 FROM (" + p + ") EXCEPT SELECT * FROM (" + g + ")) THEN 0 "
            "WHEN EXISTS(SELECT 1 FROM (" + g + ") EXCEPT SELECT * FROM (" + p + ")) THEN 0 "
            "ELSE 1 END;"
        )
...
def run_eval(gold_file, pred_file, db_path, mode, save_pred_sqls, num_cpus=60, timeout=30):
    """
    主评估函数：根据指定模式（greedy_search / major_voting / pass@k）评估预测 SQL 的准确率。

    参数
    ----
    gold_file : str
        标注文件路径，内含标准 SQL 与数据库 ID。
    pred_file : str
        预测文件路径，内含模型生成的 SQL 列表。
    db_path : str
        数据库文件夹路径，按 db_id 组织 *.sqlite 文件。
    mode : str
        评估模式：greedy_search（取首个预测）、major_voting（多数投票）、pass@k（k 次采样任一通过）。
    save_pred_sqls : bool
        是否将最终用于评估的 SQL 列表落盘到本地 json。
    num_cpus : int, 可选
        并行评估进程数，默认 60。
    timeout : int, 可选
        单条 SQL 执行超时时间（秒），默认 30。

    返回
    ----
    tuple(float, list | None)
        (准确率, 最终 SQL 列表)。pass@k 模式下后者为 None。
    """
    ...
    # 获取问题文本：优先用 "question"，否则退到 "SpiderSynQuestion"
    try:
    ...
    if mode == "greedy_search":
        ...
        print("total pred_sqls:", len(pred_sqls), "total db_files:", len(db_files))
