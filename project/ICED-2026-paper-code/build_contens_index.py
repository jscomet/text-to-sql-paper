# Local copy of content indexing utilities (BM25 via pyserini)
from new_code.utils.db_utils import get_cursor_from_path, execute_sql_long_time_limitation
import json
import os, shutil
            ...
            try:
                ...
                for c_id, column_content in enumerate(column_contents):
                    if len(column_content) != 0 and len(column_content) <= 25:
                        all_column_contents.append({"id": "{}-**-{}-**-{}".format(table_name, column_name, c_id).lower(),"contents": column_content})
            except Exception as e:
                print(str(e))
