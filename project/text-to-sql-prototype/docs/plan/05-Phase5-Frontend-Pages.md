...
frontend-dev (Task 5.4)              tester (Task 5.5)
    │                                    │
    ├── 查询页面（核心）                   ├── 端到端集成测试
    ├── 历史记录页面                      ├── 真实后端API联调
    └── 收藏功能                         └── 测试-反馈-修复循环
...
- Task 5.5 依赖 Task 5.1、5.2、5.3、5.4 全部完成
- 都依赖阶段4的基础架构
...
**依赖**: Task 5.1、5.2、5.3、5.4 全部完成
**参考文档**: `../02-UI-Design.md`, `../05-API-Documentation.md`

#### 工作内容

1. **端到端集成测试**
   - 使用 Playwright 进行浏览器端到端测试
   - 配置真实后端 API (localhost:8000)
   - 覆盖所有前端页面核心功能
...
2. **测试场景执行**
   ...
   - **测试场景2**: 连接管理流程
     - 创建各类数据库连接（SQLite/MySQL/PostgreSQL）
     - 测试连接有效性
     - Schema 展示验证
     - 编辑和删除连接
...
3. **测试-反馈-修复循环机制**
   ...
   **流程图**:
   ...
   ┌─────────────┐    ┌─────────────┐    ┌──────▼──────┐
   │  验证修复   │←───│  修复缺陷   │←───│  发送报告   │
   │  (回归测试) │    │ (frontend-dev)│   │  (给frontend-dev)│
   └──────┬──────┘    └─────────────┘    └─────────────┘
          │
          └────────→ 测试通过 → 生成报告
   ```
   ...
   **反馈机制**:
   - **发现缺陷时**:
     1. 记录缺陷详情（页面、操作步骤、预期结果、实际结果、截图）
     2. 使用 SendMessage 向 frontend-dev 发送错误报告
     3. 标记测试状态为 `blocked`
...
| 测试项目 | 测试内容 | 通过标准 | 优先级 |
|----------|----------|----------|--------|
| 登录注册 | 登录/注册流程 | 功能完整，验证正确，错误提示友好 | P0 |
| 首页 | 数据展示、快捷入口 | 数据正确，图表显示正常，跳转正确 | P0 |
| 查询页面 | NL→SQL→执行→结果 | 完整流程可用，结果正确，加载状态明确 | P0 |
| 连接管理 | CRUD + Schema | 所有操作正常，Schema展示完整 | P0 |
| 历史记录 | 列表/收藏/重新执行 | 功能完整，分页正常，数据持久化 | P1 |
| 评测管理 | 创建/执行/查看结果 | 任务可创建，结果正确，进度更新 | P1 |
| 设置页 | 修改信息/API Key | 保存成功，即时生效 | P1 |
| 响应式 | 不同屏幕尺寸 | 布局正常，无元素溢出 | P2 |
| 性能 | 大数据量加载 | 无明显卡顿，加载时间<3s | P2 |
...
5. ✅ Test Agent 集成测试通过
   ...
   - 所有P1测试用例通过率≥90%
   - 无阻塞性缺陷（阻塞主流程的bug已修复）
   - 测试报告已生成并归档到 `docs/report/05-Phase5-Frontend/`
6. ✅ 测试-反馈-修复闭环完成（所有发现的高优先级缺陷已修复并验证）
