...
E2E 测试过程中发现的 Bug 统一记录在 `@project\text-to-sql-prototype\e2e\bugs` 目录。
...
**示例**: `BUG-001-sql-generation-result-not-displayed.md`
...
| 日期 | 操作人 | 更新内容 |
|------|--------|----------|
| YYYY-MM-DD | {姓名} | 创建 Bug 报告 |
| YYYY-MM-DD | {姓名} | 修复完成，验证通过 |
```

### Bug 修复流程

```
发现 Bug → 创建报告 → 分析原因 → 实施修复 → E2E验证 → 更新状态
     ↑                                               |
     └────────────── 回归测试不通过 ←────────────────┘
```

1. **发现 Bug**: E2E 测试过程中发现异常
2. **创建报告**: 在 `e2e/bugs/` 目录按模板创建 Bug 报告文件
3. **分析原因**: 定位根本原因，记录在报告中
4. **实施修复**: 修改代码，记录修复的文件和变更
5. **E2E 验证**: 重新执行相关测试用例验证修复
6. **更新状态**: 更新 Bug 报告状态和 `index.md` 索引

### 严重级别定义

| 级别 | 标识 | 定义 | 响应时间 |
|------|------|------|----------|
| **Critical** | 🔴 | 核心功能不可用，阻塞测试进度 | 立即修复 |
| **High** | 🟡 | 主要功能异常，有 workaround | 24小时内 |
| **Medium** | 🟢 | 次要功能问题 | 下次迭代 |
| **Low** | ⚪ | 界面/体验优化建议 | 排期处理 |
